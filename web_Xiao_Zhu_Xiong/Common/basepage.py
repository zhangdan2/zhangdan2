...
class BasePage:
    ...
    def wait_eleVisible(self,loc,img_doc="",timeout=20,frequency=0.5):
    ...
    def get_element(self,loc,img_doc=""):
        ...
        :return: WebElement对象。
        """
        logging.info("查找 %s 中的元素 %s "%(img_doc,loc))
        try:
            ele = self.driver.find_element(*loc)
            logging.info(f'查找元素成功')
            return ele
        except:
            # 日志
    ...
    def click(self,loc,img_doc,timeout=20,frequency=0.5):
        """
    ...
    def xuanting(self,loc,img_doc,timeout=20,frequency=0.5):
        # 1、等待元素可见
        self.wait_eleVisible(loc,img_doc,timeout,frequency)
        # 2、找元素
        el = self.get_element(loc,img_doc)
        # 3、再操作
        logging.info(" 鼠标悬停 {}".format(loc))
        try:
            ...
            # 日志
            logging.exception("鼠标悬停元素失败")
            # 截图
            self.save_web_screenshot(img_doc)
            raise

    # 文本输入
    def input(self,loc,img_doc,*args):
        # 1、等待元素可见
